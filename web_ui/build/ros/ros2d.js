...
ROS2D.ImageMap = function (options) {
  console.log("imageMap fn is called")
  options = options || {};
...
ROS2D.ImageMapClient = function (options) {
  console.log("they are calling image map client")
  var that = this;
  options = options || {};
  ...
  rosTopic.subscribe(function (message) {
    console.log("if we subscribed", message)
...
ROS2D.OccupancyGridSrvClient = function (options) {
  console.log("they are calling the servie")
  var that = this;
  options = options || {};
  var ros = options.ros;
  var service = "/map_server/map" //options.service || "/static_map";
  this.rootObject = options.rootObject || new createjs.Container();
...
ROS2D.NavigationRoute = function (options) {
  var that = this;
  options = options || {};
  var size = options.size || 10;
  ...
  // create the shape
  createjs.Shape.call(this, graphics);
  console.log("NavigationRoute - ros2d");
};
ROS2D.NavigationRoute.prototype.__proto__ = createjs.Shape.prototype;
